...
function quick_run_simulations
% This is a quick run master run control file that calls all of the finite-sample simulations for efficiency and robustness sections
% This is identical to run_simulations.m, except that the convergence tolerance, number of trials, and number of samples points have been reduced for faster run times
% Original values have been commented out
...
run_fig5b; % Figure 4b -- Max Efficiency Finite-Sample t-Distribution
...
function run_fig8a % Figure 8 -- Gaussian divergence vs. k, p =  5, n =   5p
...
sigma          = eye(p);
epsilon        = 0.1;
k              = [0:2, 2.5:0.5:3.5, 4:8, 10]; %[0:0.5:2.5, 2.75:0.25:3.5, 4:0.5:6, 7:10];
tol            = 1e-4; %1e-6;
...
sigma          = eye(p);
epsilon        = 0.1;
k              = [0:5, 5.5:0.5:7, 8, 9:2:15]; %[0:0.5:4, 4.2:0.25:8, 8.5:0.5:10, 11:15];
tol            = 1e-4; %1e-6;
rngFun         = @(~) GaussDist.rand(p, n, mu, sigma);
mleFun         = @(X, ~, mu0, sigma0) GaussDist.mle(X, mu0, sigma0, tol);
b              = 0.5-0.5*p/n; % Maximum breakdown point
...
sigma          = eye(p);
epsilon        = 0.1;
k              = [0:5, 5.2:0.2:6.4, 6.7, 7]; %0:0.1:7;
tol            = 1e-4; %1e-6;
rngFun         = @(~) GaussDist.rand(p, n, mu, sigma);
mleFun         = @(X, ~, mu0, sigma0) GaussDist.mle(X, mu0, sigma0, tol);
b              = 0.5-0.5*p/n; % Maximum breakdown point
